...
def _load_reader_api() -> tuple[Any, Any, Any]:
    package_root = str(Path(__file__).resolve().parents[1] / "src")
    if package_root not in sys.path:
        sys.path.insert(0, package_root)
    reader_module = import_module("opencode_history_grep.reader")
    return reader_module.HistoryReader, reader_module.SessionFilter, reader_module.open_history_database


HistoryReader, SessionFilter, open_history_database = _load_reader_api()
...
def test_session_filter_matches_root_and_child_directories_only(tmp_path: Path) -> None:
    database_path = tmp_path / "history.db"
    _create_history_db(database_path)
    ...
    sessions = reader.list_sessions(session_filter=SessionFilter(directory_prefix=Path("/repo")))

    assert {session.id for session in sessions} == {"session-1", "session-2", "session-3"}
