...
2. **Message Timeline Extraction (MANDATORY FIRST HISTORY STEP)**: Your very first history command MUST be `opencode-history-grep show --session <id> --all --type message`. This message-only pass is for phase discovery, requirement reconstruction, and macroscopic timeline reconstruction. You are PROHIBITED from using `grep` as your first command to blind-search the session.
3. **Full Execution Overview (MANDATORY SECOND HISTORY STEP)**: After the message-only pass, run `opencode-history-grep show --session <id> --all --type all`. This overview exposes tool calls and tool results so you can map execution actions to the message-level requirements before judging.
4. **Full-Text Evidence Pass Before Judgment (MANDATORY)**: Before writing any audit template or drawing any conclusion, you MUST run `opencode-history-grep show --session <id> --all --type all --full-text`. This is the evidence pass used for actual judgment. You must not rely on the truncated message-only or all-block overview for final conclusions.
5. **Targeted Deep Dive**: Only after reading the `--type all --full-text` session, if specific nodes still need more local context, use the corresponding history commands, for example `show --anchor`, to inspect those nodes. If you need to continue auditing only after a known block because the main agent performed later updates, use `opencode-history-grep show --session <id> --all --type all --from <block-id-or-index> --full-text`; add `--to <block-id-or-index>` only when the review window has a known endpoint. Do not guess.
6. **Phase Breakdown & Initial Write (TEMPLATE ONLY)**:
   - Split the session into discrete "Phases" based on user requirement changes or major error pivots identified from the message-only pass, then attach tool actions from the all-block evidence pass to the relevant phase.
   - For each Phase, include at least these fields: Requirement, Action, Evidence, Docs Impact, Knowledge DB Suggestion, Risk Point, and Verification/Verdict.
   - Generate a `run_id` for this audit run and write the template into a run-scoped audit file for this run.
   - The working file path for a fresh run must be `.sisyphus/tmp/audit/audit_{session_id}.{run_id}.md`.
   - **CRITICAL**: At this step, you are only building the template. All verification checkboxes in the file MUST be left unchecked as `[ ]`. Do not write verdicts into the checklist yet.
...
8. **Secondary Physical Verification**:
...
9. **Append Rectification Goals**: At the bottom of the same file, summarize all unresolved risks and scope drifts into a concrete rectification list.
...
- **MUST**: Use the message-only pass to reconstruct requirements and phase boundaries, then use the all-block full-text pass to judge whether tool execution matched those requirements.
...
- **MUST**: Treat `opencode-history-grep show --session <id> --all --type message` as the requirement timeline pass, `opencode-history-grep show --session <id> --all --type all` as the execution overview pass, and `opencode-history-grep show --session <id> --all --type all --full-text` as the judgment evidence pass.
...
- **NEVER**: Do not write conclusions from the truncated `--type all` overview alone. Reason: the overview is for mapping execution shape, not final judgment.
